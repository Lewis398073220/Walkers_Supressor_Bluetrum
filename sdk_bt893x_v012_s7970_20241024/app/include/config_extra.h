...
#if (BT_SCO_NR_EN || BT_SNDP_SMIC_AI_EN || BT_SCO_SMIC_AI_EN || BT_SCO_NR_USER_SMIC_EN)
#undef BT_SCO_SMIC_EN
#define BT_SCO_SMIC_EN                  1
#endif

#if (BT_SNDP_DMIC_EN || BT_SCO_DMIC_AI_EN || BT_SCO_NR_USER_DMIC_EN || BT_SNDP_FBDM_EN || BT_SNDP_DM_AI_EN)
...
#if BT_SCO_NR_EN && (BT_SNDP_SMIC_AI_EN || BT_SCO_SMIC_AI_EN)
#error "NR err: please select only one nr algorithm in BT_SCO_NR_EN, BT_SNDP_SMIC_AI_EN, BT_SCO_SMIC_AI_EN\n"
#endif
...
#if (ANC_ALG_WIND_NOISE_FF_TALK_EN + ANC_ALG_AI_WN_FF_EN + ANC_ALG_WIND_NOISE_FF_FB_EN) > 1
