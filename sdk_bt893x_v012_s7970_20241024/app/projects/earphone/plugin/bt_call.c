#include "include.h"
    ...
    static dnn_aec_ns_cb_t dnn_cb AT(.aiaec_dnn_buf);
...
void bt_sco_aiaec_dnn_init(u8 *sysclk, nr_cb_t *nr)
{
    ...
    nr->nr_type = NR_TYPE_SMIC_AI;
    memset(&dnn_cb, 0, sizeof(dnn_aec_ns_cb_t));
...
//	dnn_cb.gain_expand			   	= 1024;
	...
	dnn_cb.nlp_level                = 2;
    dnn_cb.smooth_en				= 1;
    dnn_cb.far_post_thr				= 1024;//Q8
    dnn_cb.dtd_post_thr				= 36000;//Q15
    dnn_cb.echo_gain_floor			= 100;
    nr->rfu[0]                      = 1;    //bypass hw_aec nlp

    bt_aiaec_dnn_init(&dnn_cb);
