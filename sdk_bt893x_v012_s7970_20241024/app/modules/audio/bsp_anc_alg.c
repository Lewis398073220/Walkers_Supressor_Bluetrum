...
void alg_anc_ai_wn_ff_set_ch(u8 ch);
...
#define HOWLING_FB_BUF_SIZE     4312
...
#define HOWLING_FF_BUF_SIZE     4312
...
#endif // ANC_ALG_ASM_SIM_FF_EN
#if ANC_ALG_HOWLING_FB_EN
    {ANC_ALG_HOWLING_FB,        0,          0,              HOWLING_LEN,1,      ANC_ALG_HOWLING_FB_CH,      HOWLING_LEN,     (void*)&howling_fb},
#endif // ANC_ALG_HOWLING_FB_EN
#if ANC_ALG_FIT_DETECT_FF_FB_EN
    {ANC_ALG_FIT_DETECT_FF_FB,  0,          0,              256,        1,      ANC_ALG_FIT_DETECT_CH,      256,             (void*)&anc_fit_detect},
#endif // ANC_ALG_FIT_DETECT_FF_FB_EN
#if ANC_ALG_HOWLING_FF_EN
    {ANC_ALG_HOWLING_FF,        0,          0,              HOWLING_LEN,1,      ANC_ALG_HOWLING_FF_CH,      HOWLING_LEN,     (void*)&howling_ff},
#endif // ANC_ALG_HOWLING_FF_EN
#if ANC_ALG_AI_WN_FF_EN
...
#if ANC_ALG_HOWLING_FB_EN || ANC_ALG_HOWLING_FB_EN
    [ANC_ALG_ADJUST_FB_GAIN_HOWLING]    = 0x7FFF,
#endif // ANC_ALG_HOWLING_FB_EN
#if ANC_ALG_ASM_SIM_FF_EN || ANC_ALG_ASM_FF_EN
    [ANC_ALG_ADJUST_FB_GAIN_ASM]        = 0x7FFF,
...
#if ANC_ALG_HOWLING_FB_EN
#define howling_adjust_gain         anc_alg_adjust_fb_gain[ANC_ALG_ADJUST_FB_GAIN_HOWLING]
#elif ANC_ALG_HOWLING_FF_EN
#define howling_adjust_gain         anc_alg_adjust_ff_gain[ANC_ALG_ADJUST_FF_GAIN_HOWLING]
#endif
...
NO_INLINE void anc_alg_process_do(void)
{
    ...
    if (anc_alg_todo_flag & BIT(ANC_ALG_TODO_HOWLING_FB_SET0)) {
        ...
        anc_alg_adjust_fb_gain_set_vol(0, 0, 12);               //回gain fade
        TRACE("howling_adjust_gain: %d 0\n", howling_adjust_gain);
    }
    if (anc_alg_todo_flag & BIT(ANC_ALG_TODO_HOWLING_FB_SET1)) {
        anc_alg_todo_flag &= ~BIT(ANC_ALG_TODO_HOWLING_FB_SET1);
        anc_alg_adjust_fb_gain_set_vol(0, 1, 12);               //啸叫时direct set
        TRACE("howling_adjust_gain: %d 1\n", howling_adjust_gain);
    }
#endif // ANC_ALG_HOWLING_FB_EN
    //FF防啸叫
#if ANC_ALG_HOWLING_FF_EN
...
bool anc_alg_init_callback(u8 type)
{
    ...
    if (type == ANC_ALG_HOWLING_FB) {
        return alg_anc_howling_fb_set(howling_fb_buf, sizeof(howling_fb_buf), 1);
    }
#endif // ANC_ALG_HOWLING_FB_EN
    ...
    if (type == ANC_ALG_HOWLING_FF) {
        return alg_anc_howling_ff_set(howling_ff_buf, sizeof(howling_ff_buf), 1);
    }
#endif // ANC_ALG_HOWLING_FF_EN

#if ANC_ALG_AI_WN_FF_EN
    if (type == ANC_ALG_AI_WN_FF) {
        alg_anc_ai_wn_ff_set_ch((ANC_ALG_STEREO_EN + 1));
        return true;
    }
#endif // ANC_ALG_AI_WN_FF_EN
...
//        printf("ff_gain: %d fb_gain: %d howling_reslut: %d level: %d\n", res[0], res[1], res[2], res[3]);
        if ((res[1] <= 0) && (res[1] >= ANC_GAIN_MIN_DB)) {
            s16 new_fb_gain = dvol_tbl_Q15_step0P1db[res[1] * (-10)];
            if (new_fb_gain != howling_adjust_gain) {
                howling_adjust_gain = new_fb_gain;
                if (res[2]) {
                    anc_alg_todo_flag |= BIT(ANC_ALG_TODO_HOWLING_FB_SET1);
                } else {
                    anc_alg_todo_flag |= BIT(ANC_ALG_TODO_HOWLING_FB_SET0);
...
//        printf("ff_gain: %d fb_gain: %d howling_reslut: %d level: %d\n", res[0], res[1], res[2], res[3]);
        if ((res[0] <= 0) && (res[0] >= ANC_GAIN_MIN_DB)) {
            ...
            if (new_ff_gain != howling_adjust_gain) {
                howling_adjust_gain = new_ff_gain;
                if (res[2]) {
                    anc_alg_todo_flag |= BIT(ANC_ALG_TODO_HOWLING_FF_SET1);
                } else {
                    anc_alg_todo_flag |= BIT(ANC_ALG_TODO_HOWLING_FF_SET0);
